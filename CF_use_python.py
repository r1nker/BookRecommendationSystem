...
class UserCf:
    ...
    def _cosine_sim(target_books, books):
        ...
        e.g: x = [1 0 1 1 0], y = [0 1 1 0 1](x1^2+x2^2+...)+sqrt(y1^2+y2^2+...)]
             cosine = (x1*y1+x2*y2+...) / [sqrt(len(movies1)*len(movies2)]
             那意味着 union_len(movies1, movies2) / sqrt(len(movies1)*len(movies2))
             算法的公式，return的值cosine就是相似度。用到的知识是线性代数求cos角度数。举个列子 空间上面有 2个点A,B。与
             坐标原点（0,0）组成AOB三角形，角AOB的度数越小，那么表示两个坐标点越相近。代入这个书籍推荐。
             我们这样理解：
